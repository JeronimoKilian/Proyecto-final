window.onload = function(){
  ...
  fetch("https://api.themoviedb.org/3/tv/airing_today?api_key=310e7e56f84e1bd28bcf90582689ab22&language=en-US&page=1")
  .then(function(r){
    return r.json()
  })
  .then(function(data){
    console.log(data);
      var seriesVivo = data.results;
      var ul = document.querySelector("ul.listado-series-vivo")
      var prepath = 'https://image.tmdb.org/t/p/original'
      for (var i = 0; i < seriesVivo.length; i++) {
        var li = '<li>'
            li +=  '<img src="'+prepath+seriesVivo[i].poster_path+'" alt="">'
            li +=  '<div class="uk-position-center uk-panel"><h1></h1></div>'
            li += '</li>'
